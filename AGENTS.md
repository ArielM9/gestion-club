...
- Use singleton pattern from `packages/db/src/client.ts`
- Generated client at `packages/generated/prisma`
...
### Fase 3: Sistema de Ropa por Categoría

1. Añadir campo `ropaIncluida` (JSON) a modelo `TemporadaCategoria` en schema
2. Crear action para guardar configuración de productos por categoría
3. Modificar AdminTemporadas para seleccionar productos por categoría
4. Modificar EntregaRopaModal para usar productos predefinidos por categoría

### Subida de Fotos de Jugadores

**Estado actual**: Campo `fotoUrl` existe en Prisma, UI de botón existe pero no funciona.

**Objetivo**: Permitir subir fotos de jugadores a MinIO (bucket `club-files`, carpeta `fotos/`).

**Plan detallado**:

1. **Crear API route para fotos**
   - `app/api/socios/foto/presigned/route.ts`
   - GET: generar presigned URL para upload (`fotos/{socioId}.{ext}`)
   - DELETE: eliminar foto de S3
   - Uso: mismo patrón que `app/api/documentos/upload/presigned/route.ts`

2. **Conectar FormularioSocio.tsx**
   - Añadir `<input type="file" accept="image/jpeg,image/png" />` oculto
   - Conectar botón "Subir Imagen" al input
   - Al seleccionar: obtener presigned URL → PUT a S3 → guardar URL en state

3. **Actualizar Server Action**
   - Añadir `fotoUrl` a Zod schema (`SocioSchema`)
   - Guardar en Prisma al crear/actualizar socio
   - Opcional: eliminar foto anterior al actualizar

4. **Verificar visualización**
   - SocioTable ya tiene código para mostrar foto
   - Verificar FichaCliente/FichaHeader

**Notas técnicas**:
- Solo JPEG/PNG, máx 2MB (como indica la UI)
- Bucket: `club-files`, path: `fotos/{socioId}.{ext}`
- Usar SDK AWS S3 existente en `lib/s3.ts`
